  ...
  <div id="sections01">
	...
	<div style="background-color: #fff0f0">
		...
		<table class="set" style="width: 70%;" align="center">
			<tr class="set">
				...
				<tr class="set">
					<td class="set" style="border-top: 0px;border-right: 0px;border-left: 0px;border-bottom: 0px;text-align: left;">&nbsp&nbsp&nbsp <?php echo $_SESSION['user_name'];?></td>
				</tr>
				<tr class="set">
					...
					$sql= "select TIMESTAMPDIFF(YEAR,reg_date,SYSDATE()) AS join_date_year,TIMESTAMPDIFF(MONTH,reg_date,SYSDATE()) AS join_date_month,TIMESTAMPDIFF(DAY,reg_date,SYSDATE()) AS join_date_day
					   from user where user_name='".$_SESSION['user_name']."'";
					$rslt1=mysqli_query($conn, $sql);
					...
					<?php
					include("connection.php");
					$sql="SELECT institution_name FROM user WHERE user_name='".$_SESSION['user_name']."'";
					$rslt1=mysqli_query($conn,$sql);
					...
					<?php
					include("connection.php");
					$sql="SELECT country_name FROM user WHERE user_name='".$_SESSION['user_name']."'";
					$rslt1=mysqli_query($conn,$sql);
					if(mysqli_num_rows($rslt1)!=0)
					{
						if($conn->query($sql))
		...
		<table class="set" style="width: 40%" align="center">
			<tr class="set">
				...
				<td class="set" style="text-align: center">
					<?php
					include("connection.php");
					$sql="SELECT COUNT(prob_id) AS no_of_solve FROM solve WHERE UName='".$_SESSION['user_name']."'";
					$rslt1=mysqli_query($conn,$sql);
					if(mysqli_num_rows($rslt1))
					{
						if($conn->query($sql))
			...
			<tr class="set">
				...
				<td class="set" style="text-align: center;">
					<?php
					include("connection.php");
					$sql="SELECT COUNT(submission_id) AS no_of_try FROM code WHERE U_name='".$_SESSION['user_name']."' AND verdict<>'Accepted'";
					$rslt1=mysqli_query($conn,$sql);
					if(mysqli_num_rows($rslt1))
					{
						if($conn->query($sql))
			...
			<tr class="set">
				...
				<td class="set" style="text-align: center;">
					<?php
					include("connection.php");
					$sql="SELECT COUNT(submission_id) AS no_of_solve FROM code WHERE U_name='".$_SESSION['user_name']."' AND verdict='Accepted'";
					$rslt1=mysqli_query($conn,$sql);
					if(mysqli_num_rows($rslt1))
					{
						if($conn->query($sql))
			...
			<tr class="set">
				...
				<td class="set" style="text-align: center;">
					<?php
					include("connection.php");
					$sql="SELECT COUNT(submission_id) AS no_of_submit FROM code WHERE U_name='".$_SESSION['user_name']."'";
					$rslt1=mysqli_query($conn,$sql);
					if(mysqli_num_rows($rslt1))
					{
						if($conn->query($sql))
			...
			<tr class="set">
				...
				<td class="set" style="text-align: center;">
					<?php
          			include("connection.php");
          			$sql="SELECT rank FROM ranklist WHERE uname='".$_SESSION['user_name']."'";
          			$rslt1=mysqli_query($conn,$sql);
          			if(mysqli_num_rows($rslt1)!=0)
          			{
            			if($conn->query($sql))
		...
		$sql="SELECT prob_id,problem_name
			  		 FROM solve,problems
			  		 WHERE prob_id=problem_id AND UName='".$_SESSION['user_name']."'";
		$rslt1=mysqli_query($conn,$sql);
		if(!isset($_GET['page'])) 
    	{
      		$_GET['page']=0;
		...
		if(mysqli_num_rows($rslt1)!=0)
		{
			...
			echo"<h4><a style='float: right; text-decoration: none;' href='UserStatistics.php?";
        	if(isset($_GET['page'])){
          		$k=$_GET['page'];
          		$k=$k+1;
          		echo "page=$k;'";
        	...
        	if(isset($_GET['page'])){
          		if($_GET['page']>0){
            	echo"<h4><a style='text-decoration: none' href='UserStatistics.php?";
            	$k=$_GET['page'];
            	$k=$k-1;
            	echo "page=$k;'";
	...
	?>
		...
		$sql="SELECT DISTINCT problemId,problem_name
			  		 FROM code,problems
			  		 WHERE problemId=problem_id AND U_name='".$_SESSION['user_name']."' AND verdict<>'Accepted'";
		$rslt1=mysqli_query($conn,$sql);
		if(!isset($_GET['page1'])) 
    	{
      		$_GET['page1']=0;
		...
		if(mysqli_num_rows($rslt1)!=0)
		{
			...
			echo"<h4><a style='float: right; text-decoration: none; color: #cc0044' href='UserStatistics.php?";
        	if(isset($_GET['page1'])){
          		$k=$_GET['page1'];
          		$k=$k+1;
          		echo "page1=$k;'";
        	...
        	if(isset($_GET['page1'])){
          		if($_GET['page1']>0){
            	echo"<h4><a style='text-decoration: none;color: #cc0044' href='UserStatistics.php?";
            	$k=$_GET['page1'];
            	$k=$k-1;
            	echo "page1=$k;'";
            	echo ">&nbsp&nbsp&nbsp&nbsp&nbspPrev</a></h4>";
