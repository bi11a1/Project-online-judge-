...
<head>
  ...
  <style>
    ...
    tr:nth-child(odd).set{background-color: #f3d8d8}
...
<body style="background-color: black">
  ...
  <div style="background-color: #fff0f0;">
    <h1 style="font-family: Monotype Corsiva;color:darkblue;text-align: center;">
      <b>OFFLINE FULL RANKLIST</b>
    </h1>
    <br>
    
    <table class="set" style="width: 70%" align="center">
      <tr class="set" style="background-color: #25ada6">
        ...
        <th class="set" style="border-right: 1px solid LightSteelBlue;border-left: 1px solid LightSteelBlue;width: 20%;color: #4d000b">Total Tried</th>
      </tr>
    </table>
    <br>
    <?php
    ...
    $sql="SELECT UName, COUNT(prob_id) AS total
          FROM solve
          GROUP BY UName
          ORDER BY total DESC";
      ...
      if($conn->query($sql))
      {
        ...
        while($problem=mysqli_fetch_assoc($rslt1))
        {
            $currentsolve = $problem['total'];
                ...
                $query="SELECT COUNT(problemId) AS no_of_try
                        FROM code
                        WHERE U_name='".$problem['UName']."' AND verdict<> 'Accepted'";
