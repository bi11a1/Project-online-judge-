...
<body style="background-color: black">
	...
	<div style="width: 80%; height:950px; background-color: CornflowerBlue  ; float: left;">
		...
		<center>
			...
			<h3 style="color: black ">TEAM SUBMISSIONS</h3>
		</center>
		<?php 
			include("connection.php");
			if(!$conn->connect_error){
			    
				$sql = "SELECT * FROM contestcode WHERE contest_id='".$_GET['CId']."' and team_name='".$_SESSION['teamName']."' ORDER BY csubmission_id DESC";
				$result=$conn->query($sql);
				...
				if ($result->num_rows == 0) {
					...
					echo "<table style='width:100%;'>
						...
						while($row = $result->fetch_assoc()) {
							...
							if($cnt>=($_GET['page']*20) and $cnt<(($_GET['page']*20)+20)){
								echo "<tr class='abc'>";
									$tmp=$_GET['CId'];
									$ttt=$row['csubmission_id'];
								   	echo"<td style='text-align:center'><a class='ssc' href='ShowCodeForContest.php?CId=$tmp&csubmissionId=$ttt'>".$row['csubmission_id']."</a></td>";
 				...
 				$CId=$_GET['CId'];
 				echo"<h1><a class='janina' style='float: right;' title='Next' href='MySubmissionsForContest.php?";
 				if(isset($_GET['page'])){
 					$k=$_GET['page'];
 					$k=$k+1;
 					echo "page=$k&CId=$CId;'";
 				...
 				if(isset($_GET['page'])){
 					if($_GET['page']>0){
 						echo"<h1><a title='Previous' class='janina' href='MySubmissionsForContest.php?";
 						$k=$_GET['page'];
 						$k=$k-1;
