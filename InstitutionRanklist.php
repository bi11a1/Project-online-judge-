...
<body style="background-color: black">
  ...
  <div style="background-color: #fff0f0;">
    <h1 style="font-family: Monotype Corsiva;color:darkblue;text-align: center;">
      <b>Institution wise Offline Ranklist</b>
    </h1>
    <br>
    
    <table class="set" style="width: 70%" align="center">
      <tr class="set" style="background-color:  #f2f28c">
        ...
        <th class="set" style="border-right: 1px solid LightSteelBlue;border-left: 1px solid LightSteelBlue;width: 38%;color: #4d000b">Institution</th>
        ...
        <th class="set" style="border-right: 1px solid LightSteelBlue;border-left: 1px solid LightSteelBlue;width: 20%;color: #4d000b">Total Users</th>
      </tr>
    </table>
    <br>
    <?php
    $center="center";
    $set="set";
    $user="InstitutionStat.php";
    $cnt=0;
    $rank=0;
    $solved=-1;
    include("connection.php");
    $sql="SELECT institution_name, COUNT(prob_id) AS total
          FROM user,solve
          WHERE user_name=UName AND institution_name <>''
        ...
        while($problem=mysqli_fetch_assoc($rslt1))
        {
           $currentsolve = $problem['total'];
              ...
              echo "<tr class=$set>";
                ...
                echo "<td class=$set width=38%>";echo "<a href='".$user."?name=".$problem['institution_name']."' style='color: #cc0044'>".$problem['institution_name']."</a></td>
                      <td class=$set width=20%> ".$problem['total']."</td>";
                
                $query="SELECT COUNT(user_name) AS total
                        FROM user
                        WHERE institution_name='".$problem['institution_name']."'";
